...
# Load the data
micro_data<-read.csv("https://raw.githubusercontent.com/HackBio-Internship/2025_project_collection/refs/heads/main/Python/Dataset/mcgc.tsv",header = TRUE,sep="\t")

time_points<-micro_data$time # Define time colum as time_points

#Then, it extracts specific columns, order them WT and Mutant for different strains and replicates:
...
par(mfrow = c(1,2), mar = c(5,4,2,3))  # 1 rows for each strains, 2 columns for each biological replicate 
# Depending on your setup, you can change the margins and the plot size!


# Plot for Strain_1_Rep 1 WT vs knock-out

plot(strain_df$Time, strain_df$Strain_1_Rep_1_wt, 
     type = "l", 
     col = "blue",
     xlab = "Time (mins)", 
     ylab = "OD600", 
     main = "Strain_1_Rep_1 WT vs KO", 
     lty = 1, 
     ylim = c(0, max(strain_df[,-1],
                     na.rm = TRUE)))
lines(strain_df$Time, strain_df$Strain_1_Rep_1_mutant, 
      col = "red", 
      lty = 2) # Add lines for Strain_1_Rep_1_mutant
...
plot(strain_df$Time, strain_df$Strain_1_Rep_2_wt, 
     type = "l", 
     col = "blue",
     xlab = "Time (mins)", 
     ylab = "OD600", 
     main = "Strain_1_Rep_2 WT vs KO", 
     lty = 1, 
     ylim = c(0, max(strain_df[,-1], 
                     na.rm = TRUE)))
lines(strain_df$Time, strain_df$Strain_1_Rep_2_mutant, 
      col = "gray", 
      lty = 2)

# Add legend to Strain_1_Rep_2 plot

legend("bottomright", legend = c("WT", "KO"),
       title = "strain_type",
       col = c("blue", "gray"),
       lty = c(1, 2), 
       cex = 0.6)
...
plot(strain_df$Time, strain_df$Strain_2_Rep_1_wt,
     type = "l", 
     col = "green",
     xlab = "Time (mins)", 
     ylab = "OD600", 
     main = "Strain_2_Rep_1 WT vs KO", 
     lty = 3, 
     ylim = c(0, max(strain_df[,-1], 
                     na.rm = TRUE)))
lines(strain_df$Time, strain_df$Strain_2_Rep_1_mutant, 
      col = "purple", 
      lty = 4)
...
plot(strain_df$Time, strain_df$Strain_2_Rep_2_wt, 
     type = "l", 
     col = "green",
     xlab = "Time (mins)", 
     ylab = "OD600", 
     main = "Strain_2_Rep_2 WT vs KO", 
     lty = 3, 
     ylim = c(0, max(strain_df[,-1], 
                     na.rm = TRUE)))
lines(strain_df$Time, strain_df$Strain_2_Rep_2_mutant, 
      col = "black", 
      lty = 4)
...
plot(strain_df$Time, strain_df$Strain_3_Rep_1_wt, 
     type = "l", 
     col = "cyan",
     xlab = "Time (mins)", 
     ylab = "OD600", 
     main = "Strain_3_Rep_1 WT vs KO", 
     lty = 5, 
     ylim = c(0, max(strain_df[,-1], 
                     na.rm = TRUE)))
lines(strain_df$Time, strain_df$Strain_3_Rep_1_mutant, 
      col = "orange", 
      lty = 6)
...
plot(strain_df$Time, strain_df$Strain_3_Rep_2_wt, 
     type = "l", 
     col = "cyan",
     xlab = "Time (mins)", 
     ylab = "OD600", 
     main = "Strain_3_Rep_2 WT vs KO", 
     lty = 5, 
     ylim = c(0, max(strain_df[,-1], 
                     na.rm = TRUE)))
lines(strain_df$Time, strain_df$Strain_3_Rep_2_mutant, 
      col = "orange", 
      lty = 6)
...
carrying_capacity<-function(final_od,time_points,od_values) {
  target_od <- final_od*0.8 # Calculates 80% of the maximum OD 
  ...
  return(time_for_capacity)# Explicitly return the result
}
...
for (i in 2:ncol(strain_df)) {  # Skip the time column
  final_od <- max(strain_df[[i]])
  result <- carrying_capacity(final_od, strain_df$Time, strain_df[[i]])
  capacity_df$OD_capacity[i - 1] <- result  # Store the result in the OD_capacity column
}
...
plot(1:2, # view the means only (two values for wild-type and knock-out means)
     ...
     ylim=range(c(wt_capacities, ko_capacities), na.rm = TRUE),
     cex=1.5,
...
wt_conf_interval <- c(wt_mean - (critical_t_value * wt_std_err), wt_mean + (critical_t_value * wt_std_err))
ko_conf_interval <- c(ko_mean - (critical_t_value * ko_std_err), ko_mean + (critical_t_value * ko_std_err))

# Add confidence intervals as error bars
arrows(x0 = 1, 
       y0 = wt_conf_interval[1],
       x1 = 1, y1 = wt_conf_interval[2], 
       angle = 90, 
       code = 3, 
       col = "green",
       lwd = 3,
       length = 0.1)  # wild-type CI
arrows(x0 = 2, 
       y0 = ko_conf_interval[1], 
       x1 = 2, 
       y1 = ko_conf_interval[2], 
       angle = 90, 
       code = 3, 
       col = "purple",
       lwd = 3,
       length = 0.1)  # mutant CI
...
axis(2, at = y_intervals, las = 1, cex.axis = 0.9)

# Annotate with t-test results

text(1.5, y_max - 0.05 * (y_max - y_min), # places the text near the top of the plot For legibility
     labels = paste("t =", round(t_test_result$statistic, 2), 
                    ", p =", format.pval(t_test_result$p.value, digits = 3)), 
                        cex = 0.9)
